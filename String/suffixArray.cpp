...
#define SET(x,n) memset(x,n,sizeof(x))
#define SZ(x) x.size()
//LCP[0][i]= len(LCP) of SA[i] & SA[i+1](sorted suffixes).
//RA[i][j] = Rank of suffix S[j...j+2^i] i.e. 2^i length substring starting at j.
...
void build_LCP(string& s)
{
	...
	for(int i=0;i<n-1;i++)
	{
		//Build the LCP array in O(NlogN)
